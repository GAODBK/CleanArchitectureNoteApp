...
public class NoteActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (itemId == R.id.searchButton) {
            Intent toSearch = new Intent(NoteActivity.this, Search.class);
            String[] tabsArray = new String[tabsList.size()];
            for (int i = 0; i < tabsList.size(); i++) {
                tabsArray[i] = tabsList.get(i).getTabName();
            }
    ...
    private void addTabToDataBase(int position, String tabName) {
        Uri uri = Uri.parse("content://dev.lyg.cp.note.provider/tabs");
        ContentValues values = new ContentValues();
    ...
    private void loadAlarm(String alarm, int num, int days) {
         ...
         */
        //安排闹钟服务
        Intent intent = new Intent(NoteActivity.this, AlarmService.class);
